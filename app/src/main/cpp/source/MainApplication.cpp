...
bool MainApplication::SetupCamera()
{
    ...
    const Vector3 eyePosition(0.0f, 20.0f, -20.0f);
...
{
    /*m_modelObject = m_sceneManager->CreateSceneObject("models/black_smith/black_smith.obj",
                                                      "material/black_smith.cgmat");*/
    m_modelObject = m_sceneManager->CreateSceneObject("models/test-cube.obj",
                                                      "material/Avatar.cgmat");
...
bool MainApplication::SetupFaceModel()
{
    // Set modParas, including the segmented portrait information, resource files, and face avatar effect parameters.
    AIFaceModParas modParas;
// Set segmented portrait information.
    modParas.landmarkMode = 68;
    modParas.landmarkData = { 0.0f };
    modParas.segmentImgW = 2048;
    modParas.segmentImgH = 2048;
    modParas.segmentImg = { 0 };
    // Specify resource files.
    modParas.modAPath = "./assets/ModA.om";
    modParas.modBPath = "./assets/ModB.om";
    modParas.modCPath = "./assets/ModC.om";
    modParas.shapePath = "./assets/shape.bin";
    modParas.characterPath = "./assets/shape.bin"; //character.bin
    // Set face avatar effect parameters.
    modParas.characterGender = 1; //1-male 2-female
    modParas.enableRealTex = 1; //1-realistic 2-notrealistic

    GeneratedParas genParas;
    int ret1 = GenerateFaceParas(modParas, genParas);
    if (ret1) {
        return false;
    }
